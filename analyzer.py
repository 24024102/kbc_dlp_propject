...
def scan_emails():
    ...
    for filename in email_files:
        ...
        for rule in rules:
            if re.search(rule['pattern'], content):
                print(f"   !!! MATCH FOUND: {rule['id']} (Score: +{rule['points']})")
    ...
    print("\n" + tabulate(results_for_table, headers=["File", "Score", "Status", "Detected"], tablefmt="fancy_grid"))
